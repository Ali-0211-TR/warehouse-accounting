...
use crate::{
    adapters::{
        ...
        dtos::LazyTableStateDTO,
    },
    shared::error::{Error, Result},
    shared::types::SortOrder,
    domain::{
        usecases::{order_usecases, device_config_usecase::DeviceConfigUseCase},
        entities::order_entity::{OrderFilter, OrderColumn},
    },
    shared::ctx::Ctx,
};
...
fn shorten_order_id(order_id: &str) -> String {
    if order_id.len() > 8 {
        ...
        order_id.chars().rev().take(8).collect::<String>().chars().rev().collect()
    } else {
        order_id.to_string()
    }
}
...
async fn print_receipt_impl(params: PrintReceiptDTO) -> Result<PrintResult> {
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        let text_content = html_content
            ...
            .split("<script").collect::<Vec<_>>().join("")
            .split("</script>").collect::<Vec<_>>().join("")
            .split("<style").collect::<Vec<_>>().join("")
            .split("</style>").collect::<Vec<_>>().join("")
            ...
            .split('<').map(|s| s.split('>').skip(1).collect::<Vec<_>>().join("")).collect::<Vec<_>>().join("")
            // Clean up whitespace
            .lines()
            .map(|line| line.trim())
        ...
        print_cmd
            .arg("-d").arg("Printer_POS_80")
            .arg(&txt_file_path);

        // Log the command being executed
        println!("🖨️ Executing print command: {:?}", print_cmd);

        let print_output = print_cmd.output()
        ...
        if !print_output.status.success() {
            ...
            return Err(Error::General(format!("Print command failed: {}", error_msg)));
        }
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        println!("🖨️ [Windows] Printing receipt with .NET PrintDocument (zero margins, thermal settings)...");
...
"#;
        ...
        let cut_output = std::process::Command::new("powershell")
            .args(&["-NoProfile", "-NonInteractive", "-Command", cut_script])
            .output();
        ...
        match cut_output {
            ...
            Ok(output) => {
                let error_msg = String::from_utf8_lossy(&output.stderr);
                println!("⚠️ [Windows] Paper cut command failed (non-critical): {}", error_msg);
            }
            Err(e) => {
                println!("⚠️ [Windows] Could not send paper cut command (non-critical): {}", e);
            }
        }
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        let output = Command::new("lp")
            .arg("-o").arg("media=Custom.80x297mm")
...
fn convert_image_to_escpos(image_path: &str, max_width: u32) -> Result<Vec<u8>> {
    ...
    if width > max_width {
        ...
        let resized = image::imageops::resize(&img.to_luma8(), max_width, new_height, image::imageops::FilterType::Lanczos3);
    ...
    let width_bytes = ((width + 7) / 8) as u16; // Round up to nearest byte
    ...
    println!("   Black pixels: {} ({:.1}%)", black_pixels, (black_pixels as f32 / pixel_count as f32) * 100.0);
    ...
    println!("   ESC/POS header: 1D 76 30 00 {:02X} {:02X} {:02X} {:02X}",
...
fn send_raw_bytes_to_printer(doc_name: &str, bytes: &[u8]) -> Result<()> {
    ...
    println!("🖨️ Sending {} bytes for '{}' to thermal printer", bytes.len(), doc_name);
...
$defaultPrinter = (Get-WmiObject -Query "SELECT * FROM Win32_Printer WHERE Default = TRUE").Name
$success = [RawPrinterHelper]::SendBytesToPrinter($defaultPrinter, $bytes)
...
"#,
    ...
    if !output.status.success() {
        ...
        return Err(Error::General(format!("Failed to send {}: {}", doc_name, error_msg)));
    }
...
async fn print_receipt_by_order_impl(
    ...
    let template = match params.printer_width.as_str() {
        "58mm" => device_config.receipt_template_58mm
            .clone()
            .or_else(|| Some(include_str!("../../../assets/templates/receipt_58mm.txt").to_string())),
        "80mm" => device_config.receipt_template_80mm
            .clone()
            .or_else(|| Some(include_str!("../../../assets/templates/receipt_80mm.txt").to_string())),
        _ => device_config.receipt_template_80mm
            .clone()
            .or_else(|| Some(include_str!("../../../assets/templates/receipt_80mm.txt").to_string())),
    }
    ...
    let logo_width = if params.printer_width == "58mm" { 32 } else { 48 };
    let company_name = device_config.company_name.clone().unwrap_or_else(|| "".to_string());

    // Check if logo_path is configured for image logo - ACTUALLY USE THIS FOR THERMAL PRINTING
    let logo_image_escpos = if let Some(ref logo_path) = device_config.logo_path {
        if !logo_path.is_empty() && std::path::Path::new(logo_path).exists() {
            // Use image logo - convert to ESC/POS commands
            let max_width = if params.printer_width == "58mm" { 384 } else { 576 }; // pixels
            ...
            match convert_image_to_escpos(logo_path, max_width) {
                ...
                Err(e) => {
                    eprintln!("❌ Failed to convert logo image: {}, falling back to ASCII", e);
                    None
                }
            }
        } else {
    ...
    let (qr_code, qr_image_escpos) = if let Some(ref qr_url) = device_config.qr_code_url {
        ...
        if !qr_url.is_empty() {
            ...
            let qr_size = if params.printer_width == "58mm" { 20 } else { 30 };
            ...
            let qr_escpos = match generate_qr_code_png(&qr_url_with_order) {
                Ok(qr_path) => {
                    ...
                    let max_width = if params.printer_width == "58mm" { 200 } else { 300 };
    ...
    let receipt = template
        .replace("{{logo}}", &logo)
        .replace("{{company_name}}", &device_config.company_name.unwrap_or_else(|| "".to_string()))
        .replace("{{company_address}}", &device_config.company_address.unwrap_or_else(|| "".to_string()))
        .replace("{{company_phone}}", &device_config.company_phone.unwrap_or_else(|| "".to_string()))
        .replace("{{company_email}}", &device_config.company_email.unwrap_or_else(|| "".to_string()))
        .replace("{{company_website}}", &device_config.company_website.unwrap_or_else(|| "".to_string()))
        .replace("{{company_tax_code}}", &device_config.company_tax_code.unwrap_or_else(|| "".to_string()))
        .replace("{{station_name}}", &device_config.station_name.unwrap_or_else(|| "".to_string()))
        .replace("{{shop_name}}", &device_config.shop_name.unwrap_or_else(|| "".to_string()))
        ...
        .replace("{{footer_message}}", &device_config.receipt_footer.unwrap_or_else(|| "".to_string()))
        .replace("{{qr_code}}", if cfg!(target_os = "windows") { "" } else { &qr_code }) // Windows: bitmap only, Mac: ASCII
        .replace("{{qr_code_url}}", &device_config.qr_code_url.clone().unwrap_or_else(|| "".to_string()));
    ...
    let width_mm = if params.printer_width == "58mm" { 58 } else { 72 }; // 80mm printer uses 72mm print width

    let custom_media = format!("Custom.{}x{}mm", width_mm, estimated_height_mm);

    println!("📏 Receipt size: {} lines, estimated {}mm height, using media: {}",
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        let print_output = print_cmd.output().map_err(|e| {
            Error::General(format!("Failed to execute print command: {}", e))
        })?;
    ...
    #[cfg(target_os = "windows")]
    {
        println!("🖨️ [Windows] Printing receipt for order {} to thermal printer...", params.order_id);

        // STEP 1: Send logo image (if available) BEFORE text receipt
        if let Some(ref logo_bytes) = logo_image_escpos {
            println!("🖼️ [Windows] Sending logo image ({} bytes) to thermal printer...", logo_bytes.len());
        ...
        // STEP 2: Send text receipt (existing code)
        // Read receipt content
        let receipt_content = std::fs::read_to_string(&receipt_file_path)
            .map_err(|e| Error::General(format!("Failed to read receipt file: {}", e)))?;

        // Calculate paper width in hundredths of inch based on printer_width
        let width_hundredths = match params.printer_width.as_str() {
            "58" => 228, // 58mm ≈ 2.28 inches
            "80" => 315, // 80mm ≈ 3.15 inches
            _ => 315,  // Default to 80mm
        };

        // Calculate dynamic paper height based on content
        // Courier New size 9 at 96 DPI ≈ 12 pixels per line ≈ 0.125 inches per line
        // Add extra space for footer and QR code (will be printed separately as image_paths)
        let line_count = receipt_content.lines().count();
        let height_per_line = 12.5; // hundredths of inch per line
        let padding = 50; // extra padding at bottom
        let height_hundredths = ((line_count as f32 * height_per_line) + padding as f32) as i32;

        println!("📏 [Windows] Receipt: {} lines, paper size: {}x{} (hundredths of inch)",
                 line_count, width_hundredths, height_hundredths);

        // Create PowerShell script for .NET thermal printing with zero margins
        let ps_script = format!(
            r#"
Add-Type -AssemblyName System.Drawing
Add-Type -AssemblyName System.Windows.Forms
...
# Custom paper size for thermal printer (width and height in hundredths of inch)
$paperSize = New-Object System.Drawing.Printing.PaperSize("ThermalReceipt", {}, {})
$printDoc.DefaultPageSettings.PaperSize = $paperSize
...
"#,
        ...
        println!("✅ [Windows] Print job sent for order {} with zero margins and thermal settings", params.order_id);

        // STEP 3: Send QR code image (if available) AFTER text receipt
        println!("🔍 [Windows] Checking QR code image availability...");
        if let Some(ref qr_bytes) = qr_image_escpos {
            tokio::time::sleep(tokio::time::Duration::from_millis(500)).await;
            println!("🔲 [Windows] Sending QR code image ({} bytes) to thermal printer...", qr_bytes.len());
...
"#;
        ...
        match cut_output {
            Ok(output) if output.status.success() => {
                println!("✅ [Windows] Paper cut command sent for order {}", params.order_id);
            }
            Ok(output) => {
                let error_msg = String::from_utf8_lossy(&output.stderr);
                println!("⚠️ [Windows] Paper cut failed (non-critical): {}", error_msg);
            }
            Err(e) => {
                println!("⚠️ [Windows] Could not send paper cut (non-critical): {}", e);
            }
        }
...
async fn get_printers_impl() -> Result<Vec<String>> {
    #[cfg(target_os = "windows")]
    {
        ...
        if !output.status.success() {
            let error_msg = String::from_utf8_lossy(&output.stderr);
            return Err(Error::General(format!("Failed to list printers: {}", error_msg)));
        }
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        let printers = String::from_utf8_lossy(&output.stdout)
            .lines()
            .filter_map(|line| {
                line.strip_prefix("printer ").and_then(|s| s.split_whitespace().next())
            })
            .map(|s| s.to_string())
            .collect();
    ...
    {
        Err(Error::General("Printer listing not supported on this platform".to_string()))
    }
}
...
async fn print_to_named_printer_impl(
    ...
    let items_text = order
        ...
        .map(|item| {
            ...
            format!(
                "{} x{} {} x {} = {}",
                product_name, item.count, unit, item.price, item.cost
            )
        })
        .collect::<Vec<_>>()
        .join("\n");
    ...
    let receipt = template
        ...
        .replace("{{time}}", &time)
        .replace("{{cashier_name}}", &cashier_name)
        .replace("{{customer_name}}", &order.client.as_ref().map(|c| c.name.clone()).unwrap_or_else(|| "-".to_string()))
        .replace("{{items}}", &items_text)
        .replace("{{subtotal}}", &subtotal.to_string())
        .replace("{{discount}}", &discount.to_string())
        .replace("{{tax}}", &tax.to_string())
    ...
    println!("🖨️ Printing to specific printer: {} with .NET PrintDocument", params.printer_name);
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        // Calculate paper width in hundredths of inch based on printer_width
        let width_hundredths = match params.printer_width.as_str() {
            "58mm" => 228, // 58mm ≈ 2.28 inches
            "80mm" => 315, // 80mm ≈ 3.15 inches
            _ => 315,  // Default to 80mm
        };

        // Create PowerShell script for .NET thermal printing with zero margins to named printer
        let ps_script = format!(
            r#"
Add-Type -AssemblyName System.Drawing
Add-Type -AssemblyName System.Windows.Forms
...
"#,
            receipt_content.replace("'", "''"), // Escape single quotes for PowerShell
            params.printer_name.replace("'", "''"), // Escape printer name for PowerShell
            width_hundredths,
            get_powershell_print_handler_with_wrapping()
        );
        ...
        if !output.status.success() {
            let error_msg = String::from_utf8_lossy(&output.stderr);
            println!("❌ Print to '{}' failed: {}", params.printer_name, error_msg);
            return Err(Error::General(format!("Print failed: {}", error_msg)));
        }

        println!("✅ Printed to '{}' with zero margins and thermal settings", params.printer_name);

        // Send paper cut command to Xprinter
        tokio::time::sleep(tokio::time::Duration::from_millis(500)).await;

        let cut_script = format!(
            r#"
Add-Type -TypeDefinition @'
using System;
using System.IO;
...
$cutCommand = [byte[]](0x1D, 0x56, 0x00)
[RawPrinterHelper]::SendBytesToPrinter('{}', $cutCommand)
"#,
            params.printer_name.replace("'", "''") // Escape printer name for PowerShell
        );
        ...
        match cut_output {
            Ok(output) if output.status.success() => {
                println!("✅ [Windows] Paper cut sent to '{}'", params.printer_name);
            }
            Ok(output) => {
                let error_msg = String::from_utf8_lossy(&output.stderr);
                println!("⚠️ [Windows] Paper cut failed (non-critical): {}", error_msg);
    ...
    Ok(PrintResult {
        success: true,
        message: format!("Receipt printed to '{}' for order {}", params.printer_name, order_params.order_id),
    })
}
...
pub struct PrintProductLabelDTO {
    ...
    pub unit: String,
    pub label_width: String, // "58mm" or "80mm"
}

#[command]
pub async fn print_product_label(
    params: PrintProductLabelDTO,
) -> IpcResponse<PrintResult> {
...
async fn print_product_label_impl(
    params: PrintProductLabelDTO,
) -> Result<PrintResult> {
    use std::io::Write;

    // Determine label width in characters
    let char_width = match params.label_width.as_str() {
        "58mm" => 32,
        "80mm" => 48,
        _ => 32,
    };
    ...
    let name_lines = wrap_text(&params.product_name, char_width);
    for line in name_lines {
        label_content.push_str(&center_text(&line, char_width));
        label_content.push('\n');
    }

    label_content.push('\n');

    // Barcode representation (simple ASCII representation)
    // For actual barcode printing, you would use ESC/POS barcode commands
    // For now, we'll print the barcode number clearly
    label_content.push_str(&center_text(&format!("CODE: {}", params.barcode), char_width));
    label_content.push('\n');

    // Simple ASCII barcode representation
    let barcode_visual = generate_ascii_barcode(&params.barcode, char_width);
    label_content.push_str(&barcode_visual);
    label_content.push('\n');

    // Price
    label_content.push_str(&center_text(&format!("Price: {:.2} UZS", params.price), char_width));
    label_content.push('\n');

    // SKU if available
    if let Some(ref sku) = params.sku {
        label_content.push_str(&center_text(&format!("SKU: {}", sku), char_width));
        label_content.push('\n');
    }

    // Unit
    label_content.push_str(&center_text(&format!("Unit: {}", params.unit), char_width));
    label_content.push('\n');

    // Date
    let date = chrono::Local::now().format("%d.%m.%Y").to_string();
    label_content.push_str(&center_text(&date, char_width));
    label_content.push('\n');
    ...
    // Write to temporary file
    let temp_dir = std::env::temp_dir();
    let label_file_path = temp_dir.join(format!("label_{}.txt", uuid::Uuid::new_v4()));

    println!("🏷️  Creating label file: {}", label_file_path.display());
    println!("📝 Label content:\n{}", label_content);

    let mut file = std::fs::File::create(&label_file_path)
        .map_err(|e| Error::General(format!("Failed to create label file: {}", e)))?;

    file.write_all(label_content.as_bytes())
        .map_err(|e| Error::General(format!("Failed to write label content: {}", e)))?;

    println!("✅ Label file created successfully");    // Print the file
    #[cfg(target_os = "windows")]
    {
        // Use PowerShell to print on Windows
        let ps_script = format!(
            r#"Get-Content -Path '{}' -Raw | Out-Printer -Name '{}'"#,
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        println!("📄 File path: {}", label_file_path.display());

        // Use lp command (same as receipts) instead of lpr
        let mut print_cmd = Command::new("lp");
        print_cmd
            .arg("-d")
            .arg(&params.printer_name)
        ...
        // Check print queue to confirm job was sent
        let queue_output = std::process::Command::new("lpq")
            .output();

        if let Ok(queue_result) = queue_output {
            let queue_msg = String::from_utf8_lossy(&queue_result.stdout);
            println!("� Print queue status:\n{}", queue_msg);
        }

        println!("✅ Label sent to printer '{}'", params.printer_name);
    }    #[cfg(target_os = "linux")]
    {
        let output = std::process::Command::new("lp")
            .arg("-d")
    ...
    // Cleanup - delay longer to ensure print completes
    let label_file_path_clone = label_file_path.clone();
    tokio::spawn(async move {
        tokio::time::sleep(tokio::time::Duration::from_secs(10)).await;
        match std::fs::remove_file(&label_file_path_clone) {
            Ok(_) => println!("🗑️  Cleaned up label file"),
            Err(e) => println!("⚠️  Failed to cleanup label file: {}", e),
        }
    });

    println!("✅ Label print request completed for '{}'", params.product_name);
...
async fn print_to_xprinter_impl(
    ...
    let mut port = serialport::new(&params.port_name, 9600)
        ...
        .open()
        .map_err(|e| Error::General(format!("Failed to open printer port {}: {}", params.port_name, e)))?;
    ...
    if let Some(ref logo_path) = device_config.logo_path {
        if !logo_path.is_empty() && std::path::Path::new(logo_path).exists() {
            let max_width = if params.printer_width == "58mm" { 384 } else { 576 };
            match convert_image_to_escpos(logo_path, max_width) {
                Ok(logo_bytes) => {
    ...
    if let Some(ref company_name) = device_config.company_name {
        if !company_name.is_empty() {
            ...
            escpos_data.extend_from_slice(company_name.as_bytes());
    ...
    escpos_data.extend_from_slice(order_line.as_bytes());

    let date_time = order.d_created.format("%Y-%m-%d %H:%M:%S").to_string();
    let date_line = format!("Date: {}\n", date_time);
    escpos_data.extend_from_slice(date_line.as_bytes());

    if let Some(ref client) = order.client {
        let client_line = format!("Client: {}\n", client.name);
        escpos_data.extend_from_slice(client_line.as_bytes());
    }
    ...
    for (index, item) in order.items.iter().enumerate() {
        ...
        escpos_data.extend_from_slice(item_line.as_bytes());
        ...
        escpos_data.extend_from_slice(price_line.as_bytes());
    }    escpos_data.extend_from_slice(b"--------------------------------\n");
    ...
    escpos_data.extend_from_slice(total_line.as_bytes());
    // Bold off
    ...
    if let Some(ref footer) = device_config.receipt_footer {
        ...
        escpos_data.extend_from_slice(footer.as_bytes());
        escpos_data.push(0x0A);
    }
    ...
    if let Some(ref qr_url) = device_config.qr_code_url {
        if !qr_url.is_empty() {
            let qr_url_with_order = qr_url.replace("{{order_id}}", &params.order_id);
            escpos_data.extend_from_slice(&[0x1B, 0x61, 0x01]); // Center
            escpos_data.extend_from_slice(qr_url_with_order.as_bytes());
            escpos_data.push(0x0A);
        }
    }
    ...
    Ok(PrintResult {
        success: true,
        message: format!("Receipt printed to Xprinter on {} for order {}", params.port_name, params.order_id),
    })
}
...
async fn print_escpos_to_system_printer_impl(
    ...
    let payment_details = if order.payments.is_empty() {
        ...
        order
            ...
            .map(|p| {
                let payment_type_translated = translate_payment_type(&p.payment_type.to_string());
                format!("{}: {}", payment_type_translated, p.summ.round_dp(2))
            })
            .collect::<Vec<_>>()
            .join("\n")
    };
    ...
    // Company info
    let company_name = device_config.company_name.clone().unwrap_or_else(|| "".to_string());
    let company_address = device_config.company_address.clone().unwrap_or_else(|| "".to_string());
    let company_phone = device_config.company_phone.clone().unwrap_or_else(|| "".to_string());
    let company_tax_code = device_config.company_tax_code.clone().unwrap_or_else(|| "".to_string());
    let footer_message = device_config.receipt_footer.clone().unwrap_or_else(|| "".to_string());

    // Replace placeholders in template
    let receipt_text = template
        .replace("{{logo}}", "") // Logo will be added as image
        .replace("{{company_name}}", &company_name)
        .replace("{{company_address}}", &company_address)
        .replace("{{company_phone}}", &company_phone)
        .replace("{{company_tax_code}}", &company_tax_code)
        .replace("{{order_id}}", &shorten_order_id(&params.order_id))
        .replace("{{date}}", &date)
        .replace("{{time}}", &time)
        .replace("{{cashier_name}}", &cashier_name)
        ...
        .replace("{{items}}", &items_text)
        .replace("{{subtotal}}", &subtotal.round_dp(2).to_string())
        .replace("{{discount}}", &discount.round_dp(2).to_string())
        .replace("{{tax}}", &tax.round_dp(2).to_string())
        .replace("{{total}}", &total.round_dp(2).to_string())
        .replace("{{payment_details}}", &payment_details)
        .replace("{{footer_message}}", &footer_message)
        .replace("{{qr_code}}", "") // QR code will be added as image
        .replace("{{qr_code_url}}", ""); // URL will be added as image

    println!("📝 Receipt text generated from template");
    ...
    println!("✅ Receipt text added ({} bytes UTF-8)", receipt_bytes.len());
    ...
    println!("   First 50 bytes: {:02X?}", &escpos_data[..50.min(escpos_data.len())]);
    ...
    #[cfg(target_os = "macos")]
    {
        println!("🖨️  Sending ESC/POS data to printer '{}' using lpr -o raw", params.printer_name);
        ...
        println!("✅ ESC/POS data sent successfully to '{}'", params.printer_name);
    }

    #[cfg(target_os = "windows")]
    {
        // Windows: Use PowerShell RawPrinterHelper
        println!("🖨️  Sending ESC/POS data to printer '{}' using RawPrinterHelper", params.printer_name);
        ...
        let escpos_base64 = base64::Engine::encode(&base64::engine::general_purpose::STANDARD, &escpos_bytes);
...
"#,
        ...
        if !output.status.success() {
            let stderr = String::from_utf8_lossy(&output.stderr);
            return Err(Error::General(format!("PowerShell command failed: {}", stderr)));
        }
    ...
    {
        return Err(Error::General("ESC/POS printing to system printer is only supported on macOS and Windows".to_string()));
    }
    ...
    Ok(PrintResult {
        success: true,
        message: format!("Receipt with logo printed to '{}' for order {}", params.printer_name, params.order_id),
    })
}
...
fn embed_image_in_pdf(
    ...
    use printpdf::{Image, ImageTransform, ImageXObject, Mm, Px, ColorSpace, ColorBits};
    ...
    println!("   Original dimensions: {}x{} pixels", img_width, img_height);
    ...
    println!("   Target dimensions: {:.1}x{:.1}mm", target_width_mm, target_height_mm);
    ...
    println!("✅ Image added to PDF layer at ({:.1}, {:.1})mm, display size {:.1}x{:.1}mm",
...
async fn print_pdf_receipt_impl(
    ...
    println!("📦 Order loaded: {} items, {} payments", order.items.len(), order.payments.len());
    ...
    let mut receipt_text = template
        .replace("{{company_name}}", device_config.company_name.as_deref().unwrap_or(""))
        .replace("{{company_address}}", device_config.company_address.as_deref().unwrap_or(""))
        .replace("{{company_phone}}", device_config.company_phone.as_deref().unwrap_or(""))
        .replace("{{company_tax_code}}", device_config.company_tax_code.as_deref().unwrap_or(""))
        .replace("{{order_id}}", &shorten_order_id(&params.order_id))
        .replace("{{date}}", &date)
        .replace("{{time}}", &time)
        .replace("{{cashier_name}}", &cashier_name)
        ...
        .replace("{{payment_details}}", &payment_details)
        .replace("{{footer_message}}", device_config.receipt_footer.as_deref().unwrap_or(""))
        .replace("{{qr_code_url}}", "");
    ...
    let receipt_text = receipt_text
        ...
        .collect::<Vec<_>>()
        .join("\n");
    println!("📝 Template rendered, {} lines", receipt_text.lines().count());

    // 5. Create PDF
    let width_mm = if params.printer_width == "58mm" { 58.0 } else { 80.0 };
    ...
    if let Some(ref qr_url) = device_config.qr_code_url {
        if !qr_url.is_empty() {
            ...
            let qr_url_with_order = qr_url.replace("{{order_id}}", &shorten_order_id(&params.order_id));
            println!("🔲 Generating QR code for: {}", qr_url_with_order);
    ...
    doc.save(&mut BufWriter::new(
        File::create(&pdf_path)
            .map_err(|e| AppError::General(format!("Failed to create PDF file: {}", e)))?,
    ))
    ...
    #[cfg(target_os = "windows")]
    {
        println!("🖨️  Opening PDF in default viewer for printing");
...
async fn test_image_print_impl(params: TestImagePrintDTO) -> Result<PrintResult> {
    ...
    if !image_exists {
        return Err(Error::General(format!("Image file not found at: {}", params.image_path)));
    }
    ...
    let max_width = if params.printer_width == "58mm" { 384 } else { 576 };
    println!("🔧 Converting image to ESC/POS (max width: {} pixels)...", max_width);
    ...
    let mut port = serialport::new(&params.port_name, 9600)
        ...
        .map_err(|e| {
            ...
            Error::General(format!("Failed to open printer port {}: {}", params.port_name, e))
        })?;
    ...
    Ok(PrintResult {
        success: true,
        message: format!("Test print sent to {} - Check printer for output", params.port_name),
    })
}
...
#[command]
pub async fn test_pattern_print(
    printer_name: String,
) -> IpcResponse<PrintResult> {
...
async fn test_pattern_print_impl(
    printer_name: String,
) -> Result<PrintResult> {
    ...
    println!("📏 Pattern: {}x{} pixels ({} bytes per row)", width, height, width_bytes);
    ...
    {
        Err(Error::General("Pattern test only supported on macOS currently".to_string()))
    }
}

#[command]
pub async fn test_usb_image_print(
    params: TestUsbImagePrintDTO,
) -> IpcResponse<PrintResult> {
...
async fn test_usb_image_print_impl(
    params: TestUsbImagePrintDTO,
) -> Result<PrintResult> {
    ...
    if !std::path::Path::new(&params.image_path).exists() {
        return Err(Error::General(format!("Image file not found: {}", params.image_path)));
    }
    ...
    println!("✅ Image converted successfully ({} bytes)", escpos_image_data.len());
...
"#,
    ...
    #[cfg(not(any(target_os = "macos", target_os = "windows")))]
    {
        return Err(Error::General("USB printer testing is only supported on macOS and Windows".to_string()));
    }
    ...
    Ok(PrintResult {
        success: true,
        message: format!("Test image sent to USB printer '{}' - Check for output!", params.printer_name),
    })
}
